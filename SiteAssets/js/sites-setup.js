...
var AttemptConfiguration = function () {
    ...
    LoadSodByKey('jquery-2.2.1.min.js', function () {
        jQuery.when.apply(jQuery, [ProvisioningSettings(), WebProperties()]).then(function (settings, propertyBag) {
            if (propertyBag.PnPTemplateApplied !== "0")
                return;
            var templateId = propertyBag.PnPTemplateId;
            jQuery.ajax({ url: _spPageContextInfo.siteAbsoluteUrl + "/_api/web/lists/getByTitle('" + settings.PnPTemplateListName + "')/Items(" + templateId + ")/File?$select=ServerRelativeUrl", type: 'get', headers: { accept: 'application/json;odata=verbose' } }).done(function (result) {
